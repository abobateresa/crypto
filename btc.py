...
TOKEN = '8528170692:AAH7RI_MXEkt1Qq9NIhTePaR_nc4RS6eh40'  # !!! ЗАМЕНИТЕ НА ВАШ ТОКЕН !!!
...
def generate_key_and_address():
    """Генерирует HEX, WIF и сжатый BTC-адрес."""
    private_key_bytes = os.urandom(32)

    # 1. Генерация BTC-адреса (Сжатый формат)
    sk = ecdsa.SigningKey.from_string(private_key_bytes, curve=CURVE)
    vk = sk.verifying_key
    public_key_bytes = b'\x02' + vk.to_string()[:32]

    key_hash = hash160(public_key_bytes)
    versioned_hash = ADDRESS_VERSION + key_hash
    checksum = double_sha256(versioned_hash)[:4]
    bitcoin_address = base58.b58encode(versioned_hash + checksum).decode('utf-8')

    # 2. Генерация WIF-ключа (Сжатый формат: начинается с K или L)
    # WIF Version prefix (0x80) + Private Key (32 bytes) + Compression flag (0x01)
...
async def setup_database():
    """Асинхронное создание базы данных с полем WIF."""
...
async def save_address_async(address_data, balance):
    """Асинхронное сохранение адреса, HEX и WIF."""
    async with aiosqlite.connect(DATABASE_NAME) as db:
        await db.execute('''
...
async def get_all_addresses_async():
    async with aiosqlite.connect(DATABASE_NAME) as db:
        async with db.execute(
                'SELECT address, private_key, wif_key, balance FROM generated_addresses ORDER BY timestamp DESC') as cursor:
...
async def total_balance_callback(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    addresses = await get_all_addresses_async()
    # Обратите внимание: addresses теперь содержит 4 элемента (address, hex, wif, balance)
...
async def main() -> None:
    ...
    # ИСПРАВЛЕНО: Инициализация Bot для aiogram 3.7+
